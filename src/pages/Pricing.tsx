...
const teacherPlans: PricingPlan[] = [{
  ...
  features: ["Unlimited students", "Unlimited teacher accounts", "Advanced analytics dashboard", "Dedicated support", "Custom essay prompts", "White-label option"],
  userRole: "teacher"
}];
...
const PricingPage: React.FC = () => {
  ...
  return <div className="container mx-auto px-4 py-16 sm:px-6 lg:px-8">
      ...
      {user && <Tabs defaultValue={activeTab} value={activeTab} onValueChange={setActiveTab} className="mx-auto mb-8 w-fit">
          <TabsList>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 lg:gap-8">
        {plans.map(plan => <PricingCard key={plan.name} plan={plan} isCurrentPlan={subscribed && plan.name === subscriptionTier} />)}
      </div>
    </div>;
};
